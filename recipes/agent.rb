...
directory node['logstash']['agent']['log_dir'] do
  action :create
  mode "0755"
  owner node['logstash']['user']
  group node['logstash']['group']
...
logrotate_app "logstash" do
  ...
  not_if {
    node['logstash']['agent']['use_runit']
  }
end

if node['logstash']['agent']['use_runit']
  include_recipe "runit"
  runit_service "logstash_agent" do
    cookbook          node["logstash"]["agent"]["runit_cookbook"]
    run_template_name node["logstash"]["agent"]["runit_run_template"]
    log_template_name node["logstash"]["agent"]["runit_log_template"]
  end
