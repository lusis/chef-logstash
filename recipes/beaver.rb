...
node.default['logstash']['beaver']['server_query'] = "roles:#{node['logstash']['beaver']['server_role']} AND chef_environment:#{node.chef_environment}"
...
if Chef::Config[:solo]
  ...
  logstash_server_results = search(:node, node['logstash']['beaver']['server_query'])
